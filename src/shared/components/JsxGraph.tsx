...
import { COORD } from "../../utils/types/DataUtil.jsx";
...
export const JSXGraphCanvas = ({
  ...
  return <div id={id} className="jxgbox" style={style} />;
};
...
class JSXPoint implements JSXObject {
  ...
  setAttribute: (style: JSXPointStyle) => void = (style) =>
    this.jsxobj.setAttribute(style);
}
...
class JSXLine implements JSXObject {
  ...
  getObject: () => any = () => this.jsxobj;
  setAttribute: (style: JSXLineStyle) => void = (style) =>
    this.jsxobj.setAttribute(style);
}
...
class JSXAngle implements JSXObject {
  ...
  getObject: () => any = () => this.jsxobj;
  setAttribute: (style: JSXAngleStyle) => void = (style) =>
    this.jsxobj.setAttribute(style);
}
...
export class JSXGraph {
  ...
  init = () => {
    ...
    if (!canvasDiv) {
      console.warn("Failed to find canvas. Retry...");
      setTimeout(this.init, 100);
      return;
    }
    canvasDiv.innerHTML = "";
    this.board = window.JXG.JSXGraph.initBoard(this.id, {
      boundingbox: [-8, 8, 8, -8],
    });
  ...
  private create = (
    ...
    this.execute(() => {
      ...
      if (classobj == null) {
        console.error("Failed to find jsx element " + type);
        return;
      }

      var item = this.board.create(
        type,
        position,
        styleMap(id, defaultStyle, style)
      );
  ...
  createAngle = (
    ...
    this.require([p1, p2, p3], (...objList) =>
      this.create("angle", id, objList, {}, style)
    );
}
