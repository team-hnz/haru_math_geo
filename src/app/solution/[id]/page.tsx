...
import {
  ...
  JSXGraph,
  JSXGraphLib,
} from "../../../shared/components/JsxGraph";
...
function parse_solution(id: string, source: string): solution {
  var sol = new solution(id, source);
    ...
    fetch(`http://harugeo_api.zevoers.dev/solution/${params.id}`, {
      ...
      .then((result) => {
        if (result.content) {
          ...
          setPsol(parse_solution(params.id, result.content));
        ...
        {loaded ? (
          psol == undefined ? (
            ...
            <>
              ...
              <div className="w-[100vmin] h-[100vmin] p-6 absolute right-0 top-0 z-1">
                <Graph sol={psol} />
              </div>
              <JSXGraphLib />
            </>
...
export const ViewSol = ({ parsed_sol }: { parsed_sol: solution }) => {
  ...
  return (
    <div
      className={
        isSolBarOpened
          ? `flex fixed h-full left-[0px] top-0 items-top bg-white`
          : `flex fixed h-full left-[-460px] top-0 items-top bg-white`
      }
    >
